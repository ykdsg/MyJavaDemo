...
public class LC79word_search {
    ...
    public boolean exist(char[][] board, String word) {
        ...
        for (int i = 0; i < row; i++) {
            for (int j = 0; j < col; j++) {
                if (check(board, word, i, j, 0, readed)) {
                    return true;
                }
            }
        }
    ...
    private boolean check(char[][] board, String word, int i, int j, int start, boolean[][] readed) {
        ...
        readed[i][j] = true;
        for (int[] step : steps) {
            if (check(board, word, i + step[0], j + step[1], start + 1, readed)) {
                return true;
            }
        }
        readed[i][j] = false;
